    ...
    <div className="min-h-screen bg-slate-100 text-slate-950 lg:flex">
      <aside className="bg-slate-950 text-slate-300 lg:fixed lg:inset-y-0 lg:w-64">
        <div className="flex h-16 items-center justify-between border-b border-white/10 px-5">
          <Brand inverse />
          <span className="rounded-md border border-white/10 bg-white/5 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">{demoMode ? "Demo · not saved" : membership.role}</span>
        </div>
        <div className="flex items-center justify-between px-4 py-3 lg:block lg:px-3 lg:py-6">
          <div className="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-2.5 lg:mb-6">
            ...
            <span className="min-w-0">
              <span className="block truncate text-sm font-semibold text-white">{membership.organizationName}</span>
        ...
        <div className="hidden border-t border-white/10 p-4 lg:absolute lg:inset-x-0 lg:bottom-0 lg:block">
          <div className="flex items-center gap-3">
            ...
            <span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-white">{viewer.displayName}</span><span className="text-xs capitalize text-slate-400">{membership.role}</span></span>
