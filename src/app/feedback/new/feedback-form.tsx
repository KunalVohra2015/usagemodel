...
type FormErrors = Partial<Record<"organization" | "title" | "description" | "sourceUrl" | "screenshot", string>>;
...
export function FeedbackForm({
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!String(data.get("organization") ?? "").trim()) nextErrors.organization = "Select or add a destination company.";
  ...
  if (submitted) {
    return (
      <div className="rounded-3xl border border-emerald-200 bg-white p-7 text-center shadow-xl shadow-slate-900/5 sm:p-12" role="status">
        ...
        <p className="mt-6 text-sm font-semibold text-emerald-700">Prototype complete</p>
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-8" noValidate>
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800" role="note">
        <strong>Prototype submission:</strong> company search and creation may use the directory, but feedback entered below is not saved yet.
      </div>
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        ...
        <div className="mt-6 grid gap-6 sm:grid-cols-2">
          <CompanySelector companies={companies} initialCompanyId={initialCompanyId} onSelect={(company) => { setSelectedCompany(company); setErrors((current) => ({ ...current, organization: undefined })); }} error={errors.organization} />
          <label className="text-sm font-semibold text-slate-800 sm:col-span-2">Feedback type<select name="type" className={inputClass} defaultValue="feature_request"><option value="feature_request">Feature request</option><option value="bug">Bug</option><option value="confusing_experience">Confusing experience</option><option value="other">Other</option></select></label>
