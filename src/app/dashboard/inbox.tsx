...
export function Inbox() {
  ...
  const filtered = useMemo(() => {
    ...
    return mockFeedback.filter((item) => {
      const matchesQuery = !search || `${item.title} ${item.description} ${item.submitter.name}`.toLowerCase().includes(search);
  ...
  return (
    <>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {countCards.map((card) => {
          const count = mockFeedback.filter((item) => item.status === card.status).length;
        ...
        {filtered.length > 0 ? (
          <div>
            ...
            <ul className="divide-y divide-slate-100">
              {filtered.map((item) => <li key={item.id}><Link href={`/dashboard/feedback/${item.id}`} className="group grid gap-4 px-4 py-4 transition hover:bg-indigo-50/40 sm:px-5 md:grid-cols-[minmax(0,1fr)_10rem_9rem_7rem] md:items-center"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`size-2 shrink-0 rounded-full ${item.status === "submitted" ? "bg-indigo-500" : "bg-transparent"}`} aria-hidden="true" /><h3 className="truncate text-sm font-semibold text-slate-900 group-hover:text-indigo-700">{item.title}</h3></div><div className="mt-1 flex items-center gap-2 pl-4.5 text-xs text-slate-500"><span>{typeLabels[item.type]}</span><span>·</span><span className="truncate">{item.pageTitle}</span></div></div><div className="flex items-center gap-2"><span className="grid size-7 shrink-0 place-items-center rounded-full bg-slate-200 text-[10px] font-bold text-slate-700">{item.submitter.initials}</span><span className="truncate text-sm text-slate-700">{item.submitter.name}</span></div><div><StatusBadge status={item.status} /></div><div className="text-xs text-slate-500">{item.submittedAt}</div></Link></li>)}
            </ul>
          </div>
        ) : (
