...
export default async function DashboardFeedbackPage({
  ...
  const organizationName = realItem?.organizationName ?? mockItem!.organization;
  const createdAt = realItem?.createdAt ?? mockItem!.submittedAt;
  const submitter = realItem?.submitter ?? {
    displayName: mockItem!.submitter.name,
    initials: mockItem!.submitter.initials,
  };
  const history = realItem
    ...
    : [...mockItem!.timeline].reverse().map((event, index) => ({
  ...
  return (
    <>
      ...
      <main className="mx-auto grid max-w-7xl items-start gap-6 p-4 sm:p-7 xl:grid-cols-[minmax(0,1fr)_22rem] lg:p-10">
        <div className="space-y-6">
          <article className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="p-5 sm:p-7">
              ...
              {demoMode && mockItem!.hasScreenshot && <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-8 text-center text-xs font-semibold text-slate-500">Private demo screenshot</div>}
            </div>
          </article>
          ...
          {item.officialResponse && <section className="rounded-2xl border border-teal-200 bg-teal-50 p-5 shadow-sm sm:p-7"><p className="text-xs font-bold uppercase tracking-wider text-teal-700">Official {organizationName} response</p><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-teal-950">{realItem?.officialResponse?.body ?? mockItem!.officialResponse?.body}</p></section>}
        ...
        <aside className="xl:sticky xl:top-6"><FeedbackControls feedbackId={item.id} initialStatus={item.status} initialResponse={realItem?.officialResponse?.body ?? mockItem!.officialResponse?.body} canManageStatus={demoMode || membership?.role === "admin"} demoMode={demoMode} /></aside>
      </main>
    </>
  );
}
